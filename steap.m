...
rows = size(map_matrix, 1);
cols = size(map_matrix, 2);
...
field = signedDistanceField2D(map_matrix, cell_size);
sdf = PlanarSDF(origin_point2, cell_size, field);

% % plot sdf
figure(2)
plotSignedDistanceField2D(field, 0, 0, cell_size);
title('Signed Distance Field')


%% settings
total_time_sec = 1;
total_time_step = 100; %how many variable factors
check_inter = 5;
...
figure(1), hold on
plotEvidenceMap2D(map_matrix, origin_x, origin_y, cell_size);
title('Layout')
...
for i = 0 : total_time_step
    ...
    if i > 0
        ...
        graph.add(GaussianProcessPriorPose2(key_pos1, key_vel1, ...
            key_pos2, key_vel2, delta_t, Qc_model));close all
clear
